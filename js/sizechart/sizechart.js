var sizeChart = Class.create({
    ...
    findSize: function () {
		...
		this.lengthh = this.form['lengthh'];
		this.lengthbtm = this.form['lengthbtm'];
		this.shoulder = this.form['shoulder'];
		...
		var lengthhIndex = this.closestValueIndex(this.lengthh.getValue(), this.lengthhValues);
        this.table.rows[lengthhIndex].cells[4].addClassName('current');
		var lengthbtmIndex = this.closestValueIndex(this.lengthbtm.getValue(), this.lengthbtmValues);
        this.table.rows[lengthbtmIndex].cells[5].addClassName('current');
		var shoulderIndex = this.closestValueIndex(this.shoulder.getValue(), this.shoulderValues);
        this.table.rows[shoulderIndex].cells[6].addClassName('current');
        /* ---- Find max index for size selection ---------*/
        var array = [bustIndex, waistIndex, hipIndex, lengthhIndex, lengthbtmIndex, shoulderIndex];
    ...
    chartValues: function () {
        ...
        for (var rowIndex = 0; rowIndex < rows.length; ++rowIndex) {
            ...
            for (var index = 0; index < cells.length; ++index) {
				...
				if (index === 4)
                    this.lengthhValues[rowIndex] = parseInt(value);
				if (index === 5)
                    this.lengthbtmValues[rowIndex] = parseInt(value);
				if (index === 6)
                    this.shoulderValues[rowIndex] = parseInt(value);
            }
        }
    },
    ...
    clearForm: function () {
        ...
        this.form['hip'].clear();
		this.form['lengthh'].clear();
		this.form['lengthbtm'].clear();
		this.form['shoulder'].clear();
    },
